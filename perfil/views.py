...
def home(request):
    ...
    'total_entradas' : total_entradas, 'total_saidas' : total_saidas, 'percentual_gastos_essenciais' : int(percentual_gastos_essenciais),
    'percentual_gastos_nao_essenciais' : int(percentual_gastos_nao_essenciais) })

def gerenciar(request):
    #busca as contas do banco de dados
    contas = Conta.objects.all()
    categorias = Categoria.objects.all()

    total_contas = calcula_total(contas, 'valor')
    
    #print(f"Total da soma: ", total_conta)
...
def cadastrar_banco(request):
    print(request.POST.get('apelido'))
...
#Função para validação de nomes e espaço em branco
    if len(apelido.strip()) == 0 or len(valor.strip()) == 0:
        messages.add_message(request, constants.ERROR, 'Preencha todos os campos!')
        return redirect('/perfil/gerenciar/')

#TAREFA: REALIZAR MAIS VALIDAÇÕES
...
def cadastrar_categoria(request):
    ...
    # TAREFA DE CASA: Realizar validações de categoria
...
def update_categoria(request, id):
    ...
    #Ao pressionar os botões da categoria, ele muda

    if categoria.essencial == True:
